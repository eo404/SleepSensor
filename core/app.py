...
from ui.hud import HUD, draw_safety_panel, draw_environment_panel, draw_safety_message
...
class App:
    ...
    def run(self):
        ...
        for camera_idx in range(3):  # Try camera indices 0, 1, 2
            ...
            if test_cap.isOpened():
                ...
                if ret and test_frame is not None:
                    ...
                    print(
                        f"[App] Camera {camera_idx} opened but cannot read frames")
                    test_cap.release()
            else:
                test_cap.release()
        ...
        print(
            f"[App] Camera resolution: {actual_width:.0f}x{actual_height:.0f}")
        ...
        try:
            ...
            while True:
                ...
                if not ret or frame is None:
                    frame_fail_count += 1
                    print(
                        f"[App] Frame grab failed (attempt {frame_fail_count}/{max_frame_failures})")

                    if frame_fail_count >= max_frame_failures:
                        print(
                            "[App] Too many frame grab failures – camera may have disconnected")
                        break
                ...
                draw_environment_panel(frame, self.env)
                draw_safety_message(frame, self.safety)
